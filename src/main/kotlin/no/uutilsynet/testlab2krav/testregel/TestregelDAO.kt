...
class TestregelDAO(
  ...
  object TestregelParams {
    ...
    val getNextTestregelUtfallIdSql =
        "select coalesce(max(id), 0) + 1 from \"testlab2_krav\".\"testregel_utfall\"".trimMargin()
        ...
        val definition =
            if (modus == TestregelModus.manuellForenkla) {
              ...
              ManuellForenklaTestregelDefinition(
                  description = schema,
                  utfall = getTestregelUtfallForTestregel(id),
                  helptext = helptext,
              )
            } else {
  ...
  fun createTestregel(testregelInit: TestregelInit): Int {
    ...
    params.addValue(
        "helptext",
        (testregelInit.definition as? ManuellForenklaTestregelDefinition)?.helptext,
    )
      ...
      jdbcTemplate
          .update(
              ...
              mapOf(
                  ...
                  "helptext" to
                      (testregel.definition as? ManuellForenklaTestregelDefinition)?.helptext,
              ),
          )
          .also {
  ...
  private fun getTestregelUtfallForTestregel(testregelId: Int): List<TestregelUtfall> =
      jdbcTemplate.query(
          TestregelParams.getTestregelUtfallForTestregelSql,
          mapOf("testregelId" to testregelId),
      ) { rs, _ ->
  ...
  private fun deleteTestregelUtfallForTestregel(testregelId: Int): Int =
      jdbcTemplate.update(
          TestregelParams.deleteTestregelUtfallForTestregelSql,
          mapOf("testregelId" to testregelId),
      )
  ...
  private fun createTestregelUtfall(testregelId: Int, utfall: TestregelUtfall): Int {
    val id =
        jdbcTemplate.queryForObject(
            TestregelParams.getNextTestregelUtfallIdSql,
            emptyMap<String, Any>(),
            Int::class.java,
  ...
  private fun extractUtfallFromSchema(schema: String): List<TestregelUtfall> =
      runCatching {
            ...
            utfallNodes.mapIndexed { index, node ->
              val beskrivelse =
                  node.get("beskrivelse")?.asString()
                      ?: node.get("description")?.asString()
                      ?: node.get("utfall")?.asString()
                      ?: ""

              val testresultat =
                  parseUtfall(
                      node.get("testresultat")?.asString() ?: node.get("resultat")?.asString()
                  )
              ...
              TestregelUtfall(
                  ...
                  default =
                      node.get("default")?.asBoolean()
                          ?: node.get("erDefault")?.asBoolean()
                          ?: false,
              )
            }
          }
  ...
  private fun toTestregelUtfall(id: Int, utfallValue: String, erDefault: Boolean): TestregelUtfall {
    ...
    val beskrivelse =
        parsed?.get("beskrivelse")?.asString()
            ?: parsed?.get("description")?.asString()
            ?: parsed?.get("utfall")?.asString()
            ?: utfallValue

    val testresultat =
        parseUtfall(parsed?.get("testresultat")?.asString() ?: parsed?.get("resultat")?.asString())
