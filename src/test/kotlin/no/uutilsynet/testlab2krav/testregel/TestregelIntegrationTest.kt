 ...
 class TestregelIntegrationTests(
    ...
    @MockitoBean
    private lateinit var kravregisterClient: KravDAO
 ...
 URI("https://www.uutilsynet.no/wcag-standarden/111-ikke-tekstlig-innhold-niva/87")
    ...
    fun updateTestregel() {
        ...
        if (updatedTestregel != null) {
            client.put()
                .uri("/v1/testreglar")
                .body(updatedTestregel)
                .exchange()
                .expectStatus().isOk
        }

        val fetchedTestregel = client.get()
            .uri(location)
            .exchange()
            .expectStatus().isOk
            .expectBody<Testregel>()
    ...
    private fun createDefaultTestregel(): URI =
        client.post()
            ...
            .returnResult<Void>()
            .responseHeaders.location
            ?.also { deleteThese.add(idFromLocation(it)) }
